...
public class BTreeInternalPage extends BTreePage {
    ...
    public byte[] getPageData() {
        ...
        // padding
        int zerolen = BufferPool.getPageSize() - (INDEX_SIZE + 1 + header.length + td.getFieldType(keyField).getLen() * (keys.length - 1) + INDEX_SIZE * children.length);
    ...
    private void deleteEntry(BTreeEntry e, boolean deleteRightChild) throws DbException {
        ...
        if (rid == null) throw new DbException("tried to delete entry with null rid");
        ...
        if (!isSlotUsed(rid.getTupleNumber())) throw new DbException("tried to delete null entry.");
        if (deleteRightChild) {
    ...
    public void updateEntry(BTreeEntry e) throws DbException {
        RecordId rid = e.getRecordId();
        if (rid == null) throw new DbException("tried to update entry with null rid");
        if ((rid.getPageId().getPageNumber() != pid.getPageNumber()) || (rid.getPageId().getTableId() != pid.getTableId()))
            throw new DbException("tried to update entry on invalid page or table");
        if (!isSlotUsed(rid.getTupleNumber())) throw new DbException("tried to update null entry.");

        for (int i = rid.getTupleNumber() + 1; i < numSlots; i++) {
            if (isSlotUsed(i)) {
                if (keys[i].compare(Op.LESS_THAN, e.getKey())) {
                    throw new DbException("attempt to update entry with invalid key " + e.getKey() + " HINT: updated key must be less than or equal to keys on the right");
                }
                break;
            }
        }
        for (int i = rid.getTupleNumber() - 1; i >= 0; i--) {
            if (isSlotUsed(i)) {
                if (i > 0 && keys[i].compare(Op.GREATER_THAN, e.getKey())) {
                    throw new DbException("attempt to update entry with invalid key " + e.getKey() + " HINT: updated key must be greater than or equal to keys on the left");
                }
    ...
    public void insertEntry(BTreeEntry e) throws DbException {
        ...
        if (emptySlot == -1) throw new DbException("called insertEntry on page with no empty slots.");
        ...
        for (int i = 0; i < numSlots; i++) {
            if (isSlotUsed(i)) {
                if (children[i] == e.getLeftChild().getPageNumber() || children[i] == e.getRightChild().getPageNumber()) {
                    if (i > 0 && keys[i].compare(Op.GREATER_THAN, e.getKey())) {
                        throw new DbException("attempt to insert invalid entry with left child " + e.getLeftChild().getPageNumber() + ", right child " + e.getRightChild().getPageNumber() + " and key " + e.getKey() + " HINT: one of these children must match an existing child on the page" + " and this key must be correctly ordered in between that child's" + " left and right keys");
                    }
                    lessOrEqKey = i;
    ...
    private void markSlotUsed(int i, boolean value) {
        ...
        if (value) header[headerbyte] |= 1 << headerbit;
        else header[headerbyte] &= (0xFF ^ (1 << headerbit));
    }
    ...
    protected Field getKey(int i) throws NoSuchElementException {
        ...
        if (i <= 0 || i >= keys.length) throw new NoSuchElementException();
    ...
    protected BTreePageId getChildId(int i) throws NoSuchElementException {
        ...
        if (i < 0 || i >= children.length) throw new NoSuchElementException();
